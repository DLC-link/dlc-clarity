...
(define-public (setup-loan (btc-deposit uint) (attestor-ids (buff 32)))
    (let
      (
        ...
        (create-return (unwrap-panic (unwrap! (ok (contract-call? .dlc-manager-v1 create-dlc target (var-get protocol-wallet-address) attestor-ids)) err-contract-call-failed)))
