...
(define-constant sample-protocol-contract .sample-contract-basic-v0-1)
...
(define-public (setup-user-contract (closing-time uint) (emergency-refund-time uint))
    (let
      (
      ...
      (begin
          ...
          (unwrap! (ok (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.dlc-manager-basic-v0-1 create-dlc closing-time emergency-refund-time target nonce))) err-cant-unwrap)
      )
    )
)
...
(define-public (close-dlc (nonce uint) (outcome uint)) 
  (let (
    ...
    (begin 
      ...
      (unwrap! (ok (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.dlc-manager-basic-v0-1 close-dlc uuid outcome))) err-cant-unwrap)
    )
  )
)
